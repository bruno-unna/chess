...
* `debug [ on | off ]`: switch the debug mode of the engine on and off.
...
* `setoption name <id> [value <x>]`: this is sent to the engine when the user wants to change the internal parameters of the engine. For the `button` type no value is needed.
...
* `register`: this is the command to try to register an engine or to tell the engine that registration will be done later. This command should always be sent if the engine has sent `registration error` at program startup.
...
* `ucinewgame`: this is sent to the engine when the next search (started with `position` and `go`) will be from a different game. This can be a new game the engine should play or a new game it should analyse but also the next position from a testsuite with positions only.
...
* `bestmove <move1> [ ponder <move2> ]`: the engine has stopped searching and found the move `<move>` best in this position.
...
* `copyprotection`: this is needed for copyprotected engines. After the uciok command the engine can tell the GUI, that it will check the copy protection now. This is done by `copyprotection checking`.
...
* `info`: the engine wants to send information to the GUI. This should be done whenever one of the info has changed.
    ...
    * `refutation <move1> <move2> ... <movei>`: move `<move1>` is refuted by the line `<move2>` ... `<movei>`, i can be any number >= 1.
    ...
    * `currline <cpunr> <move1> ... <movei>`: this is the current line the engine is calculating. `<cpunr>` is the number of the cpu if the engine is running on more than one cpu. `<cpunr>` = 1,2,3.... if the engine is just using one cpu, `<cpunr>` can be omitted.
    
        If `<cpunr>` is greater than 1, always send all k lines in k strings together.
    
        The engine should only send this if the option `UCI_ShowCurrLine` is set to true.

* `option`: This command tells the GUI which parameters can be changed in the engine.

    This should be sent once at engine startup after the `uci` and the `id` commands if any parameter can be changed in the engine. The GUI should parse this and build a dialog for the user to change the settings.
    ...
    * `name <id>`: The option has the name id.
        ...
        * `<id> = NalimovCache`, type `spin`: this is the size in MB for the cache for the nalimov table bases
        ...
        * `<id> = UCI_Elo`, type `spin`: The engine can limit its strength in Elo within this interval.
        ...
        * `<id> = UCI_Opponent`, type `string`: With this command the GUI can send the name, title, elo and if the engine is playing a human or computer to the engine.
        ...
        * `<id> = UCI_EngineAbout`, type `string`: With this command, the engine tells the GUI information about itself, for example a license text, usually it doesn't make sense that the GUI changes this text with the setoption command.
        ...
        * `<id> = UCI_SetPositionValue`, type `string`: the GUI can send this to the engine to tell the engine to use a certain value in centipawns from white's point of view if evaluating this specifix position. 

            The string can have the formats:

                <value> + <fen> | clear + <fen> | clearall

    * `type <t>`: The option has type t.
